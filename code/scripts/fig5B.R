# Code for Figs 5b and S9b: contribution of species correlations to community sensitivity 
# over time and across state space for the 3-species predator-prey time series
# from Becks et al (2005) Nature
...
model <- lm(prey1 ~ correlation_contrib_disc, 
...
# plot of contribution of species correlations over 3D state space ------------------------------
fig <- plot_ly(x = ~predator, y = ~prey1, z = ~prey2, color = ~correlation_contrib_disc,
        colors = c("#567F55", "#D5D5D5", "#83C282")) %>% 
  #add_trace(data = ts, type = 'scatter3d', mode = "lines",
  #          color = I('black'), line = list(width = 1.5)) %>% 
  add_markers(data = full_df, type = 'scatter3d', marker = list(size = 7)) %>% 
  layout(scene = list(xaxis = list(title = "Predator"),
                      yaxis = list(title = "Preferred prey"),
                      zaxis = list(title = "Less-preferred prey")))

# Generate synthetic time series using a given population dynamics model
# and also generate a plot for the attractor in state space (Fig S2)
...
if (n_sp == 2) {
  # plot for 2 species
  fig <- plot_ly(data = ts[250:500, ], x = ~x1, y = ~x2,
  ...
  # save plot
  if (save_plots) {
    orca(fig, paste("figs/figSI/figS2/figSI_attractor_", func_name, "_", n_sp, "_sp_",
                    noise, "_noise", ".pdf", sep = ""), format = "pdf",
         width = 600, height = 600)
  }
} else {
  # plot for more than 2 species
  fig <- plot_ly(data = ts[250:500, ], x = ~x1, y = ~x2, z = ~x3, 
